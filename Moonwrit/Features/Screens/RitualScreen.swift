...
struct RitualScreen: View {
    ...
    private func record() {
        ...
        // Widgets, queued reminders and the lock screen card, in one call.
        store.syncOutside()
    }
...
// MARK: - Launching
//
// Wrapper so a writing session can be presented with `.fullScreenCover(item:)`,
// which needs something Identifiable rather than a pair of values.
